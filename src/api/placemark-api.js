...
export const placemarkApi = {
  ...
  findOne: {
    ...
    async handler(request) {
      try {
        const foundPlacemark = await db.placemarkStore.getPlacemarkById(
          request.params.id
        );
        if (!foundPlacemark) {
  ...
  create: {
    ...
    handler: async function (request, h) {
      try {
        ...
        const result = await db.placemarkStore.addPlacemark(
          userId,
          newPlacemark
        );
        if (result) {
        ...
        return Boom.serverUnavailable(
          "Database Error - check that a valid user id was provided"
        );
      }
    },
    tags: ["api"],
  ...
  deleteImage: {
    ...
    handler: async function (request, h) {
      try {
        ...
        // passing the image to the deleteImage function in the imageStore
        const imageDeletion = await imageStore.deleteImage(publicId);
        if (!imageDeletion) {
          return Boom.notFound(
            "Image deletion failed after calling delete method"
          );
        }
        ...
        console.log(err);
        return Boom.serverUnavailable(
          "Image deletion failed by outer try block"
        );
      }
    },
    tags: ["api"],
    description: "Delete a placemark image from Cloudinary",
    validate: { params: { image: imageSpec }, failAction: validationError },
  },
  ...
  update: {
    ...
    handler: async function (request, h) {
      try {
        ...
        const result = await db.placemarkStore.updatePlacemark(
          placemarkId,
          updatedPlacemark
        );
  ...
  getWeather: {
    ...
    notes:
      "Returns current weather data based on the latitude and longitude of the provided placemark",
  ...
  getWeatherForecast: {
    ...
    notes:
      "Returns current weather forecast based on the latitude and longitude of the provided placemark",
    validate: { payload: PlacemarkPlusSpec, failAction: validationError },
  },
};
